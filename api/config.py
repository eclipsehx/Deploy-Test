...
def load_embedder_config():
    from api.headers_helper import get_custom_headers
    ...
    for key in ["embedder", "embedder_ollama", "embedder_google"]:
        if key in embedder_config and "client_class" in embedder_config[key]:
            ...
            if class_name in CLIENT_CLASSES:
                ...
                # Inject custom headers for OpenAI client
                if class_name == "OpenAIClient":
                    if "initialize_kwargs" not in embedder_config[key]:
                        embedder_config[key]["initialize_kwargs"] = {}

                    # Merge custom headers with existing headers
                    custom_headers = get_custom_headers()
                    if "default_headers" in embedder_config[key]["initialize_kwargs"]:
                        embedder_config[key]["initialize_kwargs"]["default_headers"].update(custom_headers)
                    else:
                        embedder_config[key]["initialize_kwargs"]["default_headers"] = custom_headers
